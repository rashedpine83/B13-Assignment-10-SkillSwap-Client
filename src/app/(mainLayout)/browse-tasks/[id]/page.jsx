...
const TaskDetailsPage = async ({ params }) => {
  ...
  const proposals = await getAllMyProposals();

  // Find only current task proposal
  const existingProposal = proposals?.find(
    (proposal) =>
      String(proposal.taskId).trim() === String(task._id).trim() &&
      proposal.freelancerEmailId?.trim() === user?.email?.trim(),
  );

  const alreadySubmitted = !!existingProposal;

  return (
    <div className="max-w-6xl mx-auto p-6">
      <Link
        href="/browse-tasks"
        className="inline-flex items-center gap-2 mb-6 text-cyan-600"
      >
        <FiArrowLeft />
        Back
      </Link>
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-lg p-5">
        ...
        {!isLoggedIn || role !== "freelancer" ? (
          <div className="mt-6 rounded-2xl bg-gradient-to-r from-cyan-50 to-purple-50 p-5 text-center">
            <h3 className="font-bold">Sign in as freelancer</h3>
