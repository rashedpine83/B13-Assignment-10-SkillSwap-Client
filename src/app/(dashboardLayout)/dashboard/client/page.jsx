...
export default async function ClientOverview() {
  ...
  // Open Tasks
  const openTasks =
    tasksData?.filter((task) => task.status === "open").length || 0;

  // In Progress Tasks
  const inProgressTasks =
    tasksData?.filter((task) => task.status === "In Progress").length || 0;

  // Filter current user payments

  const clientPayments =
    paymentData?.filter((payment) => payment?.clientEmailId === emailId) || [];
      ...
      >
        ...
        {tasksData?.length > 0 ? (
          ...
          <div className="text-center py-16">
            <ClipboardList size={50} className="mx-auto text-gray-300" />

            <h3 className="text-xl font-semibold mt-4">No tasks yet</h3>
